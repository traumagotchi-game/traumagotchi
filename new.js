...
function setup() {
  ...
  capture.hide();
  // graphics = createGraphics((diameter + 20), (diameter + 20) * 3 / 4);
...
function captureWebcamGraphics() {
  ...
  for (let y = 0; y < diameter + 3 * stepSize; y += stepSize) {
...
function customizeShape() {
  if (!cubeEnabled && !sphereEnabled && !torusEnabled && !ringEnabled) {
    drawFlatGotchi();
  }
...
function menu1() {
  ...
  torusCheckbox.changed(function() {
    ...
    console.log(torusEnabled)
  });
  ringCheckbox.changed(function() {
    ringEnabled = !ringEnabled;
    console.log(ringEnabled)
  });
